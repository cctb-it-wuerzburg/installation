...
SLURM_STATE_SAVE_LOCATION=/slurm/slurmd
...
#StoragePort=1234
StoragePass=$PW_slurm
StorageUser=$DB_USER
StorageLoc=$DB_ACCOUNTING
EOF

sudo ln -s /etc/slurm-llnl/slurm.conf /usr/local/etc/slurm.conf
sudo ln -s /etc/slurm-llnl/slurmdbd.conf /usr/local/etc/slurmdbd.conf
SLURM_UID=2000
SLURM_GID=2000
echo "slurm:x:$SLURM_UID:$SLURM_GID:slurm admin:"$SLURM_STATE_SAVE_LOCATION":/bin/bash" | sudo tee --append /etc/passwd
echo "slurm:x:$SLURM_GID:slurm" | sudo tee --append /etc/group
pwconv

sudo mkdir -p "$SLURM_STATE_SAVE_LOCATION"
chown $SLURM_UID.$SLURM_GID "$SLURM_STATE_SAVE_LOCATION"
